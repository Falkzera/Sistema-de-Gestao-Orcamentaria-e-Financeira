...
def funcao_anp_producao_combustivel():
    ...
    def processar_e_salvar_csv(url, nome_arquivo_parquet):
        try:
            ...
            df = pd.read_csv(BytesIO(response.content), sep=';', encoding='utf-8')  # <- CSV, não Excel
        except requests.exceptions.RequestException as e:
            print(f"Erro ao baixar o arquivo {nome_arquivo_parquet}: {e}")
            df = pd.DataFrame()  # Ou outra ação conforme sua lógica
        
        # Trocar '-' por NaN
        # df = df.replace('-', np.nan)

        # Garantir que datetime esteja como string
        for col in df.columns:
            if pd.api.types.is_datetime64_any_dtype(df[col]):
                df[col] = df[col].astype(str)

        # Salvar em parquet na memória
